...
export function JobBoardSearch() {
  ...
  const handleSearch = async () => {
    if (!searchQuery.trim()) {
      toast({
        title: "Erro",
        description: "Digite um termo de busca (ex: tecnologia, marketing)",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      const { data, error } = await supabase.functions.invoke("jobboard-scrape", {
        body: {
      ...
      if (data.success) {
        ...
        toast({
          ...
          description: `${data.saved} empresas salvas de ${data.total} encontradas`,
        });
      } else {
  ...
  return (
    <div className="rounded-xl border border-border/50 bg-card/50 backdrop-blur-sm p-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h3 className="font-semibold text-foreground">Buscar Empresas</h3>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <div className="space-y-2">
          <Label htmlFor="jobSearchQuery">Segmento ou palavra-chave</Label>
      ...
      <div className="flex items-center gap-4 mt-6">
        ...
        {lastResult && (
          <div className="flex items-center gap-2 text-sm text-emerald-500">
            ...
            <span>
              {lastResult.saved} empresas salvas de {lastResult.total} encontradas
            </span>
          </div>
        )}
      </div>
      ...
      {isSearching && (
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium">Buscando empresas no {currentSource.name}...</p>
